"""
Comprehensive Model Evaluation Script for WangRaw ResNet50 Deepfake Detection
...
# Add parent directory to path to import from CNNDetection
...
class EvaluationOptions:
    ...
    def __init__(self, dataroot, checkpoint_path, checkpoint_dir, name, epoch='latest'):
        ...
        self.batch_size = 32
        self.loadSize = 256  # Resize to 256
        self.cropSize = 224  # Center crop to 224 (standard ResNet input)
        self.num_threads = 0  # Set to 0 to avoid pickle errors on Windows
...
def calculate_all_metrics(y_true, y_scores, threshold=0.5):
    ...
    metrics = {
        ...
        # Per-class metrics (Index 0=Fake, Index 1=Real)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate ResNet50 WangRaw model on validation dataset')
    parser.add_argument('--dataroot', type=str, required=True,
                       help='Path to validation dataset (should contain fake and real folders)')
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='Path to model checkpoint file (e.g., model_epoch_best.pth)')
    parser.add_argument('--name', type=str, default='wang_experiment',
    ...
    # Temporarily set isTrain=True to prevent automatic load_networks call in Trainer.__init__
    opt.isTrain = True
