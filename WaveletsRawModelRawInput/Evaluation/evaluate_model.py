"""
Comprehensive Evaluation Script for WaveletsRawModelRawInput Model
Loads trained wavelet-based model, runs inference, and generates publication-quality metrics and visualizations
"""
...
# evaluate_model.py is in: CNNDetection/WaveletsRawModelRawInput/Evaluation/
# We need to add:
# 1. WaveletsRawModelRawInput directory (parent)
# 2. CNNDetection directory (grandparent) for networks module
script_dir = os.path.dirname(os.path.abspath(__file__))  # Evaluation/
wavelet_dir = os.path.dirname(script_dir)  # WaveletsRawModelRawInput/
cnn_detection_dir = os.path.dirname(wavelet_dir)  # CNNDetection/

sys.path.insert(0, wavelet_dir)  # For trainer_WaveletsRawModelRawInput, data_wavelets
sys.path.insert(0, cnn_detection_dir)  # For networks module
...
class EvaluationOptions:
    ...
    def __init__(self, dataroot, checkpoint_path, checkpoint_dir, name, epoch='latest'):
        ...
        self.num_threads = 0  # 0 to avoid Windows pickle errors
        ...
        self.mode = 'binary'  # Binary mode for dataset_folder
...
def run_evaluation(model, data_loader, device):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader, desc="Evaluating"):
            # Data from wavelet dataloader: (wavelet_packets, labels)
            wavelet_packets, labels = data
            wavelet_packets = wavelet_packets.to(device)
            
            # Forward pass through model
            outputs = model.model(wavelet_packets)
...
def print_metrics_summary(metrics):
    ...
    print("\n" + "="*60)
    print("EVALUATION METRICS SUMMARY - WaveletsRawModelRawInput")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate WaveletsRawModelRawInput model')
    parser.add_argument('--dataroot', type=str, required=True,
                       help='Path to validation dataset (containing fake and real folders)')
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='Path to model checkpoint file')
    parser.add_argument('--name', type=str, default='wavelet_raw_experiment',
