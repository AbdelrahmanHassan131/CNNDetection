...
def validate_fusion(model, opt):
    """
    Validation function for Concatenation Fusion model.

    Args:
        model: ConcatenationFusionTrainer instance
        opt: validation options

    Returns:
        tuple: (acc, ap, r_acc, f_acc, y_true, y_pred)
    """
    # Create dataloader with dual inputs (same as MHA)
...
if __name__ == '__main__':
    ...
    # Validate
    acc, avg_precision, r_acc, f_acc, y_true, y_pred = validate_fusion(model, opt)

    print("=" * 80)
    print("CONCATENATION FUSION MODEL VALIDATION RESULTS")
    print("=" * 80)
