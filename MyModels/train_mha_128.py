...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("🔬 MULTI-HEAD ATTENTION FUSION TRAINING")
    print("=" * 80)
    print(f"📊 Dataset: {opt.dataroot}")
    print(f"🖼️  RGB Model: {opt.rgb_model_path}")
    print(f"🌊 Wavelet Model: {opt.wavelet_model_path}")
    print(f"🧠 Fusion Type: {getattr(opt, 'fusion_type', 'cross_attention')}")
    print(f"🎯 Attention Heads: {getattr(opt, 'num_heads', 4)}")
    print(
    ...
    for epoch in range(opt.niter):
        ...
        model.eval()
        acc, ap = validate_mha(model, val_opt)[:2]
